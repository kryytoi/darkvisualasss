...
## Промокоды
В админке (раздел «🎁 Промокоды») добавь промокод:
- **Ссылка на FunPay** — куда попадёт покупатель, применивший этот промокод (например, отдельный лот со скидкой);
- **Название промокода** — код, который покупатель вводит при покупке;
- **Скидка, %** — сколько процентов отнимет промокод от цены.

Покупатель вводит промокод на странице оплаты `/buy/<тариф>` (или заходит по ссылке `/buy/1_month?promo=КОД`) — цена сразу уменьшается на скидку, а кнопка FunPay переключается на ссылку из промокода. Каждая активация промокода считается в админке.
